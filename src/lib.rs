...
impl Server {
    #[new]
    fn new(application: PyObject, request_handler: PyObject) -> PyResult<Server> {
        let application = Arc::new(application);
        Ok(Server { application, request_handler })
    }
    ...
    fn run_with_cgi(&self, py: Python<'_>) -> PyResult<()> {
        ...
        environ.set_item("SERVER_NAME", "127.0.0.1")?;
        environ.set_item("SERVER_PORT", "8000")?;
        environ.set_item("SERVER_PROTOCOL", "HTTP/1.1")?;
        environ.set_item("SERVER_SOFTWARE", "TestServer/1.0")?;
